...
# These are "default theme_style" for certain controls - ie, Visual Studio assumes
# the theme_style will be applied, and emits a "NOT {STYLE_NAME}" if it is to be
# disabled.  These defaults have been determined by experimentation, so may
# not be completely accurate (most notably, some theme_style and/or control-types
# may be missing.
_addDefaults = {
        ...
        while (
            ...
            if self.token != "|":
                ...
                if Not:
                    ...
                    self.debug("theme_style add Not", self.token, value)
                    style &= ~value
                else:
                    list.append(self.token)
                    self.debug("theme_style add", self.token, value)
                    style |= value
            self.getToken()
