...
# could compute theme_style and render to excel without jinja2, since there is no
...
def test_styler_to_excel_unstyled(engine):
    # compare DataFrame.to_excel and Styler.to_excel when no theme_style applied
...
def test_styler_to_excel_basic(engine, css, attrs, expected):
    ...
    with tm.ensure_clean(".xlsx") as path:
        ...
        openpyxl = pytest.importorskip("openpyxl")  # test loading only with openpyxl
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            # test unstyled data cell does not have expected theme_style
            # test styled cell has expected theme_style
...
def test_styler_to_excel_basic_indexes(engine, css, attrs, expected):
    ...
    with tm.ensure_clean(".xlsx") as path:
        ...
        openpyxl = pytest.importorskip("openpyxl")  # test loading only with openpyxl
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            # test null styled index cells does not have expected theme_style
            # test styled cell has expected theme_style
...
# Note: Leaving behavior of "width"-type theme_style undefined; user should use border-width
# instead
excel_border_styles = [
    # "thin",
    "dashed",
